...
import { addDispatch, selectClass, updateOrder } from '../../../Constants/Constants';
import { useDispatch, useSelector } from 'react-redux';
import { setDataAction } from '../../../Store/Action/SetDataAction';
import { SET_API_JSON } from '../../../Store/ActionName/ActionName';
import DispatchView from './DisptachView';

function AddDispatch({ orderData }) {
    ...
    useEffect(() => {
        ...
        if (page === 1 && !removePOAvailableOrNOt) {
            ...
            var orderNewData = updateProductPOAvailableOrNot(orderData,'availableDispatch')
    ...
    if (data !== null) {
        td = data?.products?.map((ele, i) => {
            ...
            const numbers = ele?.availableDispatch;
                        ...
                        <div className='flex justify-center'>
                            {
                                availableQty !== Number(ele?.qty) ?
                                    <MyInput onChange={(e) => onChangeProdcuts(e.target.value, i, 'useQty', availableQty === 0 ? Number(ele.qty) : availableQty)} title={'Available Quantity'} value={ele?.useQty || ele?.useQty === "" ? ele?.useQty : ele?.availableDispatch?.length === 0 ? ele.qty : Number(ele.qty) - availableQty} />
                                    :
                                    0
                            }
                        </div>
    ...
    const onChangeProdcuts = (value, index, name, availableQty) => {
        ...
        if (name === 'useQty') {
            ...
            oldData.products[index].cgst = Number(oldData.products[index].availableDispatch) * Number(oldData.products[index].productVarient.gst) * Number(value) / 200
            oldData.products[index].sgst = Number(oldData.products[index].availableDispatch) * Number(oldData.products[index].productVarient.gst) * Number(value) / 200
        }
        setData(oldData)
        setRender(Date.now())
    }
                    ...
                    page === 0 ?
                        <div>
                            <div className='flex justify-end'>
                                <MyButton title={'Dipatch'} onClick={() => setPage(1)} />
                            </div>
                            <DispatchView orderData={orderData} />
                        </div>
                        :
                        page === 1 ?
                            <div>
