...
function MyInput({ placeholder, name, disable, title, error, important, uppercase, onChange, parent,value }) {
    ...
    return (
        <div>
            ...
            <input style={{ textTransform: uppercase ? 'uppercase' : '' }} disabled={disable} onChange={onChange ? onChange : (e) => onChangeText(e.target.value)} className={`${disable && 'bg-gray-200'} mt-1 w-full outline-none h-10 text-md rounded-lg border border-slate-400 placeholder:normal-case hover:border-slate-400 pl-2`} value={value?value:parent?ApiReducer?.apiJson?.[parent]?.[name]:ApiReducer?.apiJson?.[name]} name={name} placeholder={placeholder} />
